...
class DashboardRenderer {
  ...
  renderFrame() {
    ...
    // Calculate effective dimensions - use a size divisible by 16 for H.264 compatibility
    const width = Math.floor((this.config.width || 1920) / 16) * 16;  // Ensure width is divisible by 16
    const height = Math.floor((this.config.height || 1080) / 16) * 16; // Ensure height is divisible by 16

    // Resize canvas if needed to ensure dimensions are compatible with H.264
    if (this.canvas.width !== width || this.canvas.height !== height) {
      this.canvas.width = width;
      this.canvas.height = height;
      console.log(`Adjusted canvas dimensions to ${width}x${height} for encoder compatibility`);
    }
